...
export function paymentDueReminderEmail(input: {
  ...
  return {
    subject: `Payment due soon — ${input.month} · ${input.hostelName}`,
    html: emailLayout({
      heading: "Your hostel fee is due soon",
      bodyHtml: [
        paragraph(
          `Hi ${escapeHtml(input.residentName)}, your <strong>${escapeHtml(input.month)}</strong> fee at ${escapeHtml(input.hostelName)} is coming up.`,
        ),
        paragraph(
