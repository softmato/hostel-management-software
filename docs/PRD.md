...
3. Submission creates a `ServiceProvider` record with `status: PENDING_APPROVAL`, **linked to the submitter's `userId`** (today's `ServiceProvider` model has no such link — see DATABASE.md). A "registration received" email goes out (EMAIL_SYSTEM.md §6.1, unchanged). **The provider's ID card renders immediately at this point** — front/back, downloadable, same as a resident's — carrying a visible "Pending approval" tag. The card is not withheld until approval; only the tag is status-dependent (see DATABASE.md "Service Providers & Maintenance").
4. `SUPERADMIN`/`PLATFORM_MODERATOR` reviews the application and documents, same queue as today.
5. **On approval:** the linked `PUBLIC` account is upgraded to `SERVICE_PROVIDER` in place (§8.3's pattern, not Cook's synthetic-shared-account pattern — the provider has a real personal mailbox because it came from Google). A temporary password is generated and emailed alongside the approval notice (EMAIL_SYSTEM.md §6.2 — "you're approved, your card is ready, here are your credentials"), and `mustChangePassword` is set so the first login forces a password of their own choosing. The card's "Pending approval" tag clears the moment `status` becomes `APPROVED` — same card, no separate screen.
   - **On rejection:** the account stays `PUBLIC`; no credentials are issued. The rejection email (§6.3) states the reason and invites resubmission. The card's tag switches to reflect the rejected state.
