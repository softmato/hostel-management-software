...
describe("computeMonthlyDue", () => {
  it("charges the full fee when the resident moved in before the month", () => {
    expect(
      computeMonthlyDue(9000, new Date("2026-03-10T00:00:00Z"), "2026-07"),
    ).toBe(9000);
  });

  it("charges the full fee when move-in is on the 1st", () => {
    expect(
      computeMonthlyDue(9000, new Date("2026-07-01T00:00:00Z"), "2026-07"),
    ).toBe(9000);
  });
  ...
  it("pro-rates a mid-month move-in by remaining days", () => {
    ...
    expect(
      computeMonthlyDue(3100, new Date("2026-07-16T00:00:00Z"), "2026-07"),
    ).toBe(1600);
  });

  it("charges a single day for a month-end move-in", () => {
    expect(
      computeMonthlyDue(3100, new Date("2026-07-31T00:00:00Z"), "2026-07"),
    ).toBe(100);
  });

  it("charges nothing for months before the move-in", () => {
    expect(
      computeMonthlyDue(9000, new Date("2026-08-02T00:00:00Z"), "2026-07"),
    ).toBe(0);
  });
  ...
  it("handles February lengths", () => {
    ...
    expect(
      computeMonthlyDue(2800, new Date("2026-02-15T00:00:00Z"), "2026-02"),
    ).toBe(1400);
  });
});
