...
import type { StatementParser } from "@/modules/finance/statements/parsers/types";

/** Fixture text as the bytes an upload would actually deliver. */
function source(text: string): StatementBytes {
  return { bytes: Buffer.from(text, "utf8"), fileName: "statement.csv" };
}

function parse(parser: StatementParser, text: string) {
  return parser.parseTable(readStatementTable(source(text), parser.headerAnchors));
}
...
describe("Khalti CSV parser", () => {
  it("reads every row of a real export", () => {
    const rows = parse(KHALTI_CSV_PARSER, KHALTI);
  ...
  it("keeps the remark, where the reference code lives", () => {
    const [first] = parse(KHALTI_CSV_PARSER, KHALTI);
  ...
  it("refuses a transaction type it does not recognise", () => {
    ...
    expect(() => parse(KHALTI_CSV_PARSER, odd)).toThrow(/does not recognise/i);
  });

  it("refuses a part-rupee amount", () => {
    expect(() => parse(KHALTI_CSV_PARSER, KHALTI.replace('"9,000"', '"9,000.25"'))).toThrow(
      /whole rupees/i,
    );
  });

  it("carries its own version", () => {
    expect(KHALTI_CSV_PARSER.version).toBe("khalti@2");
  });
});

describe("bank CSV parser", () => {
  it("reads deposits and withdrawals into one directional column", () => {
    const rows = parse(BANK_CSV_PARSER, BANK);
  ...
  it("reads DD/MM/YYYY day-first, as declared", () => {
    const [first] = parse(BANK_CSV_PARSER, BANK);
  ...
  it("derives the same ids when the same file is read twice", () => {
    ...
    expect(parse(BANK_CSV_PARSER, BANK).map((row) => row.providerTxnId)).toEqual(
      parse(BANK_CSV_PARSER, BANK).map((row) => row.providerTxnId),
    );
  });
  ...
  it("prefers a real reference number when the bank provides one", () => {
    ...
    const rows = parse(BANK_CSV_PARSER, withRef);
  ...
  it("refuses a row with neither a deposit nor a withdrawal", () => {
    ...
    expect(() => parse(BANK_CSV_PARSER, empty)).toThrow(/neither a deposit nor a withdrawal/i);
  });

  it("refuses a row with both", () => {
    const both = BANK.replace(',,"8,000","1,20,000"', ',"100","8,000","1,20,000"');

    expect(() => parse(BANK_CSV_PARSER, both)).toThrow(/both a deposit and a withdrawal/i);
  });
});

describe("the registry keeps the three formats apart", () => {
  it("routes each fixture to its own parser", () => {
    expect(resolveParser("ESEWA", source(ESEWA))).toBe(
      resolveParser("ESEWA", source(ESEWA)),
    );
    expect(resolveParser("KHALTI", source(KHALTI)).version).toBe("khalti@2");
    expect(resolveParser("BANK", source(BANK)).version).toBe("bank@2");
  });

  it("refuses a bank file uploaded as an eSewa statement, and names what it is", () => {
    expect(() => resolveParser("ESEWA", source(BANK))).toThrow(/bank statement/i);
  });
