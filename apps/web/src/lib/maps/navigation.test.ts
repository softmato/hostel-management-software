...
import type { RouteStep } from "@/lib/maps/routing";

/**
 * Ported with the module from `apps/mobile/src/lib/navigation.test.ts` — the
 * reason the arithmetic there is trustworthy, and the reason this copy of it is.
 *
...
describe("chooseHeading", () => {
  ...
  /*
   * The desktop case, and it is the normal one on this side rather than an edge
   * case: a laptop has no magnetometer, and it is not moving, so neither source
   * has anything. Everything downstream has to read that as "stay north-up".
   */
  it("is null only when neither source has anything", () => {
    expect(chooseHeading({ compass: null, gpsHeading: null, speed: 3 })).toBeNull();
    expect(chooseHeading({ compass: null, gpsHeading: null, speed: null })).toBeNull();
  });
...
describe("headingDifference", () => {
  it("measures the short way round", () => {
    ...
    // The gate in the map canvas reads this: 358 and 2 are four degrees apart,
    // and a naive subtraction would call them 356 and repaint on every sample.
...
describe("instructionFor", () => {
  ...
  it("reads the maneuver types that contain spaces", () => {
    expect(
      instructionFor(step({ modifier: "left", name: "Bhagwati Marg", type: "end of road" })),
    ).toBe("Turn left at the end of the road onto Bhagwati Marg");
    expect(instructionFor(step({ modifier: "straight", type: "end of road" }))).toBe(
      "Continue straight",
    );
    expect(
      instructionFor(step({ modifier: "straight", name: "Maiti Devi marg", type: "new name" })),
    ).toBe("Continue onto Maiti Devi marg");
  ...
  it("keeps rather than turns at a fork, and merges", () => {
    ...
    expect(
      instructionFor(step({ modifier: "slight right", name: "Ring Road", type: "fork" })),
    ).toBe("Keep slightly right onto Ring Road");
  ...
  it("continues along a road rather than onto it", () => {
    expect(
      instructionFor(
        step({ modifier: "slight right", name: "Ananda Bhairav Marg", type: "continue" }),
      ),
...
describe("formatManeuverDistance", () => {
  it("counts down in tens near a junction", () => {
    // A 50 m-rounded formatter would give 150, 100, 50 — which cannot tell the
    // turning in front of you from the one after it.
