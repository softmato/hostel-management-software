...
import { parseRoadRoute, ROUTE_MODES, routeUrl } from "@/lib/maps/routing";

/**
 * Ported with the module from `apps/mobile/src/lib/routing.test.ts`, fixtures
...
describe("routeUrl", () => {
  ...
  /*
   * Without this flag the reply carries empty step lists and navigation has no
   * instructions to give — a Start button that opens a panel saying nothing.
...
describe("parseRoadRoute", () => {
  ...
  it("rejects a line of one point, which cannot be drawn", () => {
    expect(
      parseRoadRoute({
        code: "Ok",
        routes: [{ distance: 10, duration: 5, geometry: { coordinates: [[85.3, 27.7]] } }],
      }),
    ).toBeNull();
  });
