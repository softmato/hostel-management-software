/**
 * Keyword-based provider-role suggestion for the Maintenance & Providers screen.
 *
 * The admin types the problem in plain language ("tap in room 204 is leaking")
 * and we suggest which kind of service provider to call. It is a deliberately
 * dumb scorer — no network, no model — so the right-hand role box can update on
 * every keystroke. The admin can always override the suggestion manually.
 */
