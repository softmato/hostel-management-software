...
/**
 * Post one answer with the stored token and nothing else.
 *
...
async function postAnswer(entry: QueuedNightStatus): Promise<boolean> {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/v1/resident/night-status`, {
      body: JSON.stringify({
        note: entry.note,
    ...
    /*
     * A 401 is not a failure to retry here — it means the access token expired
     * while the notification was sitting unread, which is normal after a few
     * hours. Refreshing rotates the pair, and doing that from a background
     * process racing the foreground app is how a resident gets logged out for
     * answering a roll call (see `refresh-tokens.ts`). So it stays queued and
     * the next foreground flush, which owns the refresh, sends it.
     */
    return response.ok;
  } catch {
    return false;
...
export async function handleNightStatusResponse(
  ...
  /*
   * The notification is dismissed either way. It has been answered — leaving it
   * in the shade because the network was down would tell the resident their tap
   * did nothing, and they would answer again, or stop answering.
   */
